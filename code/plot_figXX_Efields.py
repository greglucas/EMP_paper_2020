...
def animate_fields(E_sites, ts, fs):
    ...
    grid_lats = np.linspace(lat_bounds[0]-2, lat_bounds[1]+2,
                            nlat*enhance)
    lon_mesh, lat_mesh = _mesh_grid(grid_lons, grid_lats)
    ...
    quiv_B = ax1.quiver(lonq.ravel(), latq.ravel(),
                        Bqy[0, :], Bqx[0, :],
                        transform=proj_data,
                        color='w',
                        units='inches',
    ...
    anim = animation.FuncAnimation(fig, animate, frames=[x*10 for x in range(250)], interval=10)
